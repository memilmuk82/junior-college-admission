(() => {
  ...
  const applyProfileLabels = () => {
    ...
    document.querySelectorAll("[data-derived-record-source]").forEach((input) => {
      input.value = vocationalCurrent && input.dataset.grade === "3" && input.dataset.semester === "1"
        ? "VOCATIONAL_TRAINING_RECORD"
        : "HOME_SCHOOL_RECORD";
    });
    document.querySelectorAll("[data-derived-vocational-semester]").forEach((input) => {
      input.value = vocationalCurrent && input.dataset.grade === "3" && input.dataset.semester === "1" ? "TRUE" : "FALSE";
    });
    document.querySelectorAll("[data-term-source-label]").forEach((label) => {
      label.textContent = vocationalCurrent && label.dataset.grade === "3" && label.dataset.semester === "1"
        ? "직업위탁 성적"
        : "원적교 학교생활기록부";
    });
    ...
    if (help) {
      help.textContent = vocationalCurrent
        ? "1·2학년과 선택 입력인 3학년 2학기는 원적교 학교생활기록부, 3학년 1학기만 직업위탁 성적으로 서버에서 자동 분류합니다."

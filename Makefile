...
test-unit:
	$(PYTHON) pytest tests/test_app.py tests/test_temporary_uploads.py tests/test_validate_rules.py
...
test-integration:
	...
	@status=0; TEST_DATABASE_URL=$(TEST_DATABASE_URL) $(PYTHON) pytest tests/test_database.py tests/test_migrations.py tests/test_models.py || status=$$?; \
		$(COMPOSE_TEST_ENV) docker compose --profile test rm -f -s -v db-test; \
		exit $$status

...
test-unit:
	$(PYTHON) pytest tests/test_app.py tests/test_image_imports.py tests/test_scanned_pdf_imports.py tests/test_structured_imports.py tests/test_temporary_uploads.py tests/test_text_pdf_imports.py tests/test_validate_rules.py
...
test-integration:
	...
	@status=0; TEST_DATABASE_URL=$(TEST_DATABASE_URL) $(PYTHON) pytest tests/test_confirmed_imports.py tests/test_database.py tests/test_migrations.py tests/test_models.py || status=$$?; \
		$(COMPOSE_TEST_ENV) docker compose --profile test rm -f -s -v db-test; \
		exit $$status

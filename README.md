...
Phase 14의 핵심 흐름은 `성적 직접 입력·표 붙여넣기·CSV/XLSX 업로드 → 추출값 검수 → 실제 대학·학과 선택 → 자격 판정 → 대학별 반영 평균등급과 2025 공개 입시결과 비교 → 계산 trace와 학생용·교사용 A4`입니다. 이름·학번·학교명을 요구하지 않으며, 1회 입력은 30분 만료·새 계산·완료 시 삭제됩니다. 저장할 때만 학생 또는 교사 계정으로 로그인합니다.
...
- `로그인 없이 성적 계산하기`: 빈 성적표에서 시작
- `예시 성적으로 체험하기`: 수정 가능한 합성 학생 성적을 일반 계산 엔진에 전달
- 학생 계정: 자신의 성적 저장·조회·삭제
- 교사 계정: 관리하는 학생 성적과 상담자료 저장·조회
- 관리자 계정: 공개 기준정보·규칙 검수·연도별 입시결과 import

...
현재 공개 핵심 흐름은 첫 화면에서 바로 시작하는 `성적 직접 입력·표 붙여넣기·CSV/XLSX 업로드 → 추출값 검수 → 대학·학과 검색 및 비교 선택 → 지원자격 판정 → 대학별 반영 평균등급과 2025 공개 입시결과 비교 → 계산 trace와 학생용·교사용 A4`입니다. 이름·학번·학교명을 요구하지 않으며, 1회 입력은 30분 만료·새 계산·완료 시 삭제됩니다. 공개 상담은 로그인 없이 끝까지 이용하고, 저장할 때만 학생 또는 교사 계정으로 로그인합니다.

공개 파일럿은 동양미래대학교·명지전문대학·인하공업전문대학·연성대학교의 정규화된 공개 기준정보와 기준 XLSX에서 직접 파생한 2025학년도 공개 결과 482행을 사용합니다. 공식 모집요강 근거가 확인된 규칙만 `VERIFIED_SOURCE`로 실행합니다. 명지전문대학은 비위탁 일반고 범위만 보수적으로 계산하고 위탁 이력은 `NEEDS_REVIEW`로 차단하며, 기준 XLSX의 `J18`·`J20`은 참고자료로만 남깁니다.
...
- 공개 상담: 2027학년도 일반고·졸업예정·직업위탁 참여 기준을 고정하고, 성적 우선 또는 대학 검색 우선으로 시작
- 학생 계정: 자신의 성적·상담 저장, 기존 상담 복제, 개인 BYOK 키와 AI 초안 조회·삭제
- 교사 계정: 관리하는 학생 성적·상담자료와 별도의 비식별 교내 지원 결과 등록·집계·CSV 내보내기
- 관리자 계정: 공개 기준정보·규칙 검수·연도별 입시결과 import, 근거 문서 버전과 데이터 비교·검증 결정 관리

학생·교사 가입은 역할을 명시해 승인 대기로 생성하며, 관리자 승인 후 저장 기능을 사용합니다. 공유 데모 계정은 관리자 권한을 얻지 않으며 공개 계산의 필수 입구가 아닙니다. Google OIDC는 기본 비활성 상태입니다.

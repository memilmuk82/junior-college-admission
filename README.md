...
현재 상태는 **Phase 3: 지원자격 판정 엔진 진행 중**입니다. 입력 게이트웨이 위에 학생 사실 모델, 제한형 조건 DSL, 5개 자격 상태, 설명 trace와 계산 진입 차단 코어를 제공합니다. 공식 근거와 사람 승인이 완료된 대학 규칙은 아직 없으므로 실제 대학 지원자격 결과와 점수 계산 기능은 제공하지 않습니다.

...
현재 상태는 **Phase 3: 지원자격 판정 엔진 완료**입니다. 학생 사실과 게시 규칙으로 전형별 자격을 독립 판정하고, 복수지원과 민감 결격을 별도 서비스로 평가하며 설명 trace와 계산 진입 차단 계약을 제공합니다. 공식 근거와 사람 승인이 완료된 대학 규칙은 아직 없으므로 실제 대학 지원자격 결과와 점수 계산 기능은 제공하지 않습니다.

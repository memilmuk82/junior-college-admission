...
## 2026-07-21 · Phase 19·20

- 이메일 기반 가입·로그인과 이메일 인증, 비밀번호 찾기·재설정, 회원정보 비밀번호·이메일 변경, 명시적 Google 연결·해제를 추가했다.
- 운영 공개 데모 계정은 퇴역시키고 `/demo/`에 별도 Flask·PostgreSQL·Nginx gateway·volume·세션·암호화 키를 둔 쓰기 가능한 체험 환경을 만들었다.
- 공개 대학·학과·전형과 2025·2026 입시결과는 검수 seed를 그대로 사용하고, 원본이 없는 학생·학급·성적만 합성 익명 자료로 표시했다.
- 학생은 성적·대학 계산·저장·BYOK, 교사는 학급·학생 성적·연결·상담·BYOK, 주 관리자는 교사 기능과 전체 관리 기능을 사용한다. 보조 관리자는 계정 승인만 가능하다.
- 1·2학년과 선택 입력 3학년 2학기는 원적교 성적, 3학년 1학기만 위탁 성적으로 서버에서 확정하고 관련 드롭다운을 제거했다.
- 단위 392건, PostgreSQL 통합 223건, 실제 공개 HTTPS 역할 E2E 6건과 공개 계산 E2E 4건, lint·mypy·규칙·민감자료 검사를 통과했다.
- 커밋 `a0653bc`, `1474ef4`를 push하고 운영 DB 백업·archive·격리 복원 뒤 migration `3d9c0f7a21b4`를 적용했다. 운영 DB volume과 활성 관리자 BYOK, 43개 대학·1,079개 학과·5,092개 전형, 2025 482행·2026 4,094행을 보존했다.
- 운영·체험 컨테이너를 재빌드해 모두 healthy를 확인하고 host Nginx `/demo/` proxy, token query 비기록 로그, logrotate timer를 배포했다.
- E2E가 만든 합성 자료는 체험 volume만 초기화해 활성 체험계정 4개와 기준 합성 자료로 복원했다. 운영 SMTP·실제 Google OIDC는 외부 자격증명 미설정으로 비활성이고 체험 outbox·로컬 동의에서 전체 흐름을 제공한다.

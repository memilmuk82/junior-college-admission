...
관리자 메뉴의 직접 편집과 표준 CSV는 모두 `ManagedScoreRule`을 만들고 `score_rule_to_payload()`로 같은 제한형 payload를 생성한다. payload는 출처 포함, 학기·과목 선택, 학년 가중치, 성취도, Z점수 출처·table code, 반올림과 만점을 고정 객체로만 표현한다. 임의 필드와 자유 수식을 허용하지 않는다.

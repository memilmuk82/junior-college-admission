...
Phase 7의 `build_score_rule_csv_preview()`는 canonical parser를 그대로 사용해 업로드 행을 `NEW`, `CHANGED`, `UNCHANGED`, `CONFLICT`로 분류한다. 같은 업무키·같은 버전인데 payload나 근거가 달라지면 기존 버전 덮어쓰기로 간주해 `CONFLICT`로 차단한다.

관리자가 명시적으로 선택한 `NEW`·`CHANGED` 행만 DRAFT 후보가 된다. 오류·충돌·동일 행은 선택할 수 없고 `auto_publish`는 항상 `FALSE`다. 일부 오류 행이 있어도 유효 행은 미리보기에 남지만 관리자 선택 전에는 저장하지 않는다.

Phase 7 저장 시 canonical 업무키와 대학·전형명, 근거 문서 참조·페이지·위치, 근거 상태, 변경 사유를 `ScoreRule`에 함께 보존한다. 동일 업무키·동일 버전은 DB에서도 거부하며 변경 행은 이전 규칙과 버전 계보를 남긴다. CSV 원본은 비공개 임시 검수 세션에만 보관하고 저장 또는 폐기 후 삭제한다. 신규 DRAFT는 전형 DB 연결을 추정하지 않으며, 전형 연결·근거 검증·독립 검증·골든 테스트·명시적 사람 승인 전에는 게시할 수 없다.

관리자 직접 편집 화면도 동일한 고정 필드를 개별 입력으로 제공하고 내부적으로 같은 canonical parser를 통과시킨다. 자유 JSON·수식·실행 문자열은 입력 계약에 없으며 `DRAFT` 상태만 수정할 수 있다. 전형과 공식 근거 위치는 기존 DB 항목을 명시적으로 선택해 연결하고, citation의 문서·페이지·위치가 입력 근거와 다르면 저장을 거부한다. 편집 시 독립 검증·골든 테스트·사람 승인 값은 초기화되고 `DRAFT_UPDATED` 감사 이벤트를 남긴다.

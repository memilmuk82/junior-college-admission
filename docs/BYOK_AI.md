...
- 충전 후 새 `.env.local` 키는 공식 Models API 인증에 성공했다. `gpt-5-mini`에 개인정보 없는 합성 payload를 전송해 구조화 JSON·금지 표현·숫자 검증까지 통과하는 `OPENAI_ANONYMOUS_SMOKE_PASS`를 확인했다. 키와 응답 원문은 출력하지 않았다.

...
- 이전 개발 키에서는 공식 Models API 인증이 성공했지만 생성 요청이 HTTP 429였다. 새 `.env.local` 키는 기존 셸 환경 변수를 제거하고 다시 읽어도 Models API와 `gpt-5-mini` Responses 요청 모두 HTTP 401을 반환했다. 키 값은 출력하지 않았으며 현재 유효한 운영 키가 없어 생성 성공 응답은 미검증이다.

...
- OpenAI 개발 키의 공식 Models API 인증은 성공했다. 같은 키로 비식별 합성 payload를 실제 전송했으나 `gpt-5-mini`와 `gpt-4.1-mini` 모두 HTTP 429를 반환해, 키 오류가 아닌 프로젝트 사용 한도·과금 활성화가 외부 차단 요인이다. 네트워크 도달과 오류 은닉은 확인했지만 생성 성공 응답은 미검증이다.

...
- 충전 후 새 `.env.local` 키는 기존 셸 환경 변수를 제거하고 다시 읽어도 공식 Models API 인증에 성공했다. `gpt-5-mini` 생성은 토큰 부족을 4,096 토큰으로 보완한 뒤 모델 응답까지 도달했으나, 금지 표현이 포함된 응답을 안전 검증이 차단했다. 후속 재시도는 일시적 네트워크 오류였다. 키 값과 응답 원문은 출력하지 않았으며 안전 검증을 통과한 생성 성공은 미검증이다.

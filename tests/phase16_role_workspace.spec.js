...
test('Phase 16 역할별 업무공간과 교사-학생 성적 연결은 SSR 폼으로 동작한다', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '계정 승인 요청' })).toBeVisible();
  await expect(page.getByText('보조 관리자는 승인 대기 일반 회원·학생·교사만 승인할 수 있습니다.')).toBeVisible();
  await expect(page.getByRole('link', { name: '상담 시작' })).toHaveCount(0);
